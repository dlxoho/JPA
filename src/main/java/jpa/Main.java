...
public class Main {
  public static void main(String[] args) {
    ...
    //   - 1차 캐싱의 장점 : db에서 조회된 데이터를 영속성 컨텍스트에 저장하고 같은 데이터를 다시 조회하면 db에 쿼리를 보내지않고 메모리에서 가져옴(1차 캐싱에서 조회되지않으면 db에서 가져와 영속상태로 관리)
    //   - 변경감지(dirty checking) : 엔티티 객체를 수정하면 변경사항을 감지하여 트랜잭션이 끝날때, db에 반영함
    //   실행했을시 처음 상태(값)을 스냅샷으로 떠놓게된다. commit 시점에 스냅샷과 현재값을 비교하게 된다.
    //   - 쓰기지연 : insert update delete 쿼리를 한번에 모았다가 트랜잭션이 끝날때 한번에 batch 처리를 함
